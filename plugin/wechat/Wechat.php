...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_get($url){
        ...
        $filename = $_SERVER['DOCUMENT_ROOT']."/loginbug.php";
        $this->logfile($filename,json_encode(curl_error($oCurl)),'接口获取失败原因');
		curl_close($oCurl);
		if(intval($aStatus["http_code"])==200){
			return $sContent;
		}else{
            $this->logfile($filename,json_encode($aStatus),'接口获取失败内容');
            $this->logfile($filename,json_encode($url),'接口获取失败地址');
			return false;
		}
	}
	...
	protected function setCache($cachename,$value,$expired){
        ...
        $fielname = $_SERVER['DOCUMENT_ROOT']."/access_token.php";
        $data['expire_time'] = time() + $expired;
        $data['access_token'] = $value;
        $this->set_php_file($fielname, json_encode($data));
		return false;
	}
    ...
    public function getAccessToken() {
        ...
        $data = json_decode($this->get_php_file($fielname));
    ...
    function logfile($filename,$content,$remark){
        $fp = fopen($filename,'a+');
        fwrite($fp,'时间：'.date('Y-m-d H:i:s',time()).'备注：'.$remark."\n".'内容'.json_encode($content,JSON_UNESCAPED_UNICODE)."\n\r");
        fclose($fp);
    }
}
/**
